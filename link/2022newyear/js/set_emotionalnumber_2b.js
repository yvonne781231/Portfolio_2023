(function ($){
    ...
    $.fn.aniEvent = function () {
        ...
        $('.section_buy_start').load('/dadamaster_public/event/buy_first.html?t=0420v3');
        $('.section_buy_last').load('/dadamaster_public/event/buy_last.html?t=0421v3');
        ...
        function onScroll(event) {
            ...
            //Sec5加上Fixed
            if (scrollPos >= $('#section-target').position().top && (scrollPos - 250 + $(window).height()) <= $('#Sec51').position().top) {
                $('.section_buy_last').addClass("buy--fixed");
            }
            else
                $('.section_buy_last').removeClass("buy--fixed");
        }
